<template>
    <view class="h-l-like-container">
		<view class="title">
			猜您喜欢
		</view>
        <view v-if="tempLikeList.length > 0">
            <view v-for="item of tempLikeList" :key="item.s_id">
                <scenic-list-item :item="item" @itemClick="itemClick"></scenic-list-item>
            </view>
            <view class="h-l-sell-more" @click="seeMore" v-if="showMore">查看更多</view>
        </view>
        <view v-else class="empty">暂无信息哦~</view>
    </view>
</template>
...
export default {
  ...
  watch: {
    likeList (newVal, oldVal) {
      this.tempLikeList = newVal ? newVal.slice(0, 3) : []
	  this.tempLikeList.forEach((item, index) => {
