  ...
  <view>
    <view class='activity-friend-info-container' v-if="assist">
      <view class="header">
        ...
        <text class="action" @click="inviteFriend">{{tip || '邀请好友'}}</text>
      </view>
      <view class="friend-info-wrapper" v-if="$userInfo.isLogin() && assist.join && assist.join.user && assist.join.user.length > 0">
        <image v-for="(item, index) of user" :key="index" :src="$utils.image.getImagePath(item.avatar)" mode="aspectFill"></image>
      </view>
    </view>
...
export default {
  ...
  computed: {
	...
	user () {
		if (this.assist && this.assist.join) {
			return this.assist.join.user
		} else {
			return []
		}
	}
