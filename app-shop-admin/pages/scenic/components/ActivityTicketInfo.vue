  ...
  <view>
    <view class='activity-ticket-info-container' v-if="assist">
      <view class="tip-wrapper">
		...
		<view class="price-wrapper"><text class="price">{{'￥' + scenicInfo.minPrice}}</text><text class="sales" v-if="scenicInfo.totalSales > 0">已售{{$utils.common.trasformNum(scenicInfo.totalSales)}}张</text></view>	
        <view>
			<view class="tip" v-if="showComplete && $userInfo.isLogin() && assist.join.status === 1">
			  恭喜，任务已完成
			</view>
