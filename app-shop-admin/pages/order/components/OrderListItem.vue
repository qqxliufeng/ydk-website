<template>
    <view>
		 <view class="o-l-item-card">
			...
			<view class="o-l-item-container" @click="orderItemClick">
				...
				<view class="o-l-item-info-container">
					...
					<view>
						<text>总价：￥{{item.ord_amount}}</text>
						<text style="float: right; color: #333;">详情</text>
					</view>
				</view>
			</view>
  ...
  methods: {
    orderItemClick () {
		this.$push(this.$mRouter.orderInfo({
			orderId: this.item.ord_id.toString(),
			orderType: this.item.stateModel.orderType
		}))
    },
	action1Click () {
